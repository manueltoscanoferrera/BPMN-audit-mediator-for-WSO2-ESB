...
public class TestMessageContextBuilder {
	...
	public TestMessageContextBuilder setRequireAxis2MessageContext(
			boolean requireAxis2MessageContext) {
	...
	public TestMessageContextBuilder addFileEntry(String key, String path)
	...
	public MessageContext build() throws Exception {
		...
		if (requireAxis2MessageContext) {
			synCtx = new Axis2MessageContext(
		...
		if (contentString != null) {
			parser = StAXUtils.createXMLStreamReader(new StringReader(
					contentString));
		} else if (contentFile != null) {
			parser = StAXUtils.createXMLStreamReader(new FileInputStream(
					contentFile));
		}
		...
		if (parser != null) {
			if (contentIsEnvelope) {
				...
				envelope = OMAbstractFactory.getSOAP11Factory()
						.getDefaultEnvelope();

				// TODO: don't know why this is here, but without it some unit
				// tests fail...
